workspace "Frege Language Server" {
    model {
        ...
        fregeIde   = softwareSystem "Frege Integrated Development Environment" "Provides Frege language features to develop Frege applications faster." {
            ...
            fregeGradlePlugin   = container "Frege Gradle Plugin" "Adds tasks to create, compile, run, REPL and test a Frege project to Gradle." "Java" {
                tasks      = component "Tasks" "Adds Frege specific tasks to Gradle." "Java Package"
                tooling    = component "Tooling" "Exposes Frege task configuration using the Gradle Tooling API." "Java Package, Gradle Tooling API"
            }
            fregeLanguageServer = container "Frege Language Server" "Provides Frege specific language features." "Frege and Java" {
                lsp        = component "LSP" "Receives and sends notifications, requests and responses according to the language server protocol (LSP) using the LSP4J library." "Java Package, LSP4J"
                compile    = component "Compile" "Runs the Frege compiler and manages all available compiler information, called global." "Frege Module"
                diagnostic = component "Diagnostic" "Extracts all available errors and warnings from the compiler global and returns them as LSP diagnostics." "Frege Module"
                hover      = component "Hover" "Extracts all available type signatures of functions and variables from the compiler global and returns them as LSP hover." "Frege Module"
                project    = component "Project" "Configures the Frege project with information such as external dependencies from Gradle Build Tool if available." "Java Package"
            }
        }
        ...
        fregeExtension      -> lsp "requests Frege language feature from" "Language Server Protocol (LSP)"
        ...
        project             -> tooling "extracts Frege project config from"
        project             -> compile "configures project compiler options"
        tasks               -> buildTool "adds Frege support to"
    }
    ...
    views {
        ...
        component fregeGradlePlugin "frege-gradle-plugin-components" {
            include *
            autoLayout lr
        }
